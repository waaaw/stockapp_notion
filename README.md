...
브라우저에서 http://127.0.0.1:5000 접속. 상단에 **보유 현황**(보유수량/평균단가/평가금액/
평가손익/수익률/누적실현손익/총수익(배당포함) 및 전체 합계, 보유수량이 0인 종목은 표에서 자동 제외되나
청산된 종목의 실현손익은 합계에 계속 반영됨)이 표로 보이고, 각 종목의 "매매내역" 링크를 클릭하면
해당 종목의 개별 거래를 조회·수정·삭제할 수 있는 페이지로 이동합니다. 손익은 상승(빨강)/하락(파랑)으로
구분됩니다. 차트 등 시각화는 아직 없으며 표 형태로만 제공합니다.
...
- **KIS(한국투자증권) Open API 연동**: 실제 보유/체결내역을 자동으로 가져와 수기 입력을 줄이는 상세
  계획은 [docs/plan-2026-07-05-improvements-and-kis.md](docs/plan-2026-07-05-improvements-and-kis.md)
  참고. 모의투자 계좌 등록 등 사전 준비가 필요해 이번 변경에는 포함하지 않았습니다.
